...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Retrieve the current registered users from localStorage
    const registeredUsers = JSON.parse(localStorage.getItem('registeredUsers')) || {};

    // Check if UID already exists in the registered users
    const isUIDExists = Object.values(registeredUsers).some((user) => user.uid === formData.uid);
    if (isUIDExists) {
      setError('UID already exists. Please try again.');
      setIsSubmitting(false);
      return;
    }

    // Store the new user data in registeredUsers object
    registeredUsers[formData.username] = {
      ...formData,
      role: role, // Add the user's role (patient/doctor/etc.)
    };

    // Save the updated registeredUsers object back to localStorage
